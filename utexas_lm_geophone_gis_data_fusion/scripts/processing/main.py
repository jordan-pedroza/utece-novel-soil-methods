...
f = open('../test_data/16sep2024/Geophone_Data2.csv','r')
signal1 = f.readlines()
f.close()

f = open('../test_data/16sep2024/Geophone_Data1.csv','r')
signal2 = f.readlines()
f.close()
...
plt.subplot(2, 2, 2)
plt.stem(correlation, markerfmt=" ", basefmt="-b", use_line_collection=True)
...
plt.subplot(2, 2, 4)
if(rms(signal1) > rms(signal2)):
    plt.plot(fft_freq, np.abs(fft_output1))
    plt.plot(fft_freq, np.abs(fft_output2))
else:
    plt.plot(fft_freq, np.abs(fft_output2))
    plt.plot(fft_freq, np.abs(fft_output1))
